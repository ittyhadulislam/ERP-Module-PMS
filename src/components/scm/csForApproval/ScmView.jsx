...
const ScmView = ({ showConcernApproval, showSCMRW }) => {
  ...
  const {
    data: row,
    isLoading: isScmLoading,
    refetch,
  } = useGetScmViewQuery(user?.userName);
  const [reworkPrice, { data, isSuccess }] = useReworkPriceMutation();
  const [concernApproval, { data: concernData, isSuccess: isConcernSuccess }] =
    useConcernApprovalMutation();
  ...
  useEffect(() => {
    ...
    if (concernData) successToast("Approved successfully!");
  ...
  return (
    <>
      ...
      <CustomTable
        ...
        isSuccess={isSuccess | isConcernSuccess}
      ...
      <Box sx={{ my: 1, border: "1px dashed grey", mr: "1px", mb: "1px" }}>
        ...
        >
          ...
          <span style={{ margin: "0px" }}>
            <SubmitButton
              title={"concern merchant-approval"}
              type="button"
              handleClick={handleConcernApproval}
              loading={isScmLoading}
              disabled={selectedRow.length <= 0 || showConcernApproval}
            />
            <span>
              <ReturnButton
                title={"SCM-Re-work"}
                type="button"
                handleClick={handleScmRework}
                loading={isScmLoading}
                disabled={selectedRow.length <= 0 || showSCMRW}
              />
            </span>
          </span>
        </Stack>
