...
const MM = ({ showDMApproval, showDMRW }) => {
  ...
  const {
    ...
    refetch,
  } = useGetMMViewQuery(user?.userName);
  const [reworkPrice, { data, isSuccess, isLoading }] =
    useReworkPriceMutation();

  const [
    dmmApproval,
    { data: dmmData, isSuccess: isDMMSuccess, isLoading: isDMMLoading },
  ] = useDmmApprovalMutation();
  ...
  useEffect(() => {
    refetch();
    setSelectedRows([]);
    if (dmmData) successToast("Approved successfully!");
  ...
  return (
    <>
      ...
      <CustomTable
        ...
        isSuccess={isSuccess | isDMMSuccess}
        ...
        >
          ...
          <span style={{ margin: "0px" }}>
            <SubmitButton
              ...
              disabled={selectedRow.length <= 0 || showDMApproval}
            />
            <span>
              <ReturnButton
                title={"mm/agm/dgm-Re-work"}
                type="button"
                handleClick={handleRework}
                loading={isLoading}
                disabled={selectedRow.length <= 0 || showDMRW}
              />
            </span>
          </span>
        </Stack>
