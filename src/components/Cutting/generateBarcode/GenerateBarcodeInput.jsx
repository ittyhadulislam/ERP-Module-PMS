...
const GenerateBarcodeInput = () => {
  ...
  const { data: colorData, isLoading: isColorLoading } = useGetInputColorQuery({
    ...
    style: style?.nStyleID,
    // po: po?.cPoNum,
    po: po?.cOrderNu, //rakibul 04052024
  ...
  const [deleteInputBarcode, { isSuccess: isDeleteSuccess }] =
    useDeleteInputBarcodeMutation();

  //complete Input Cut Panel

  const [completeInputBarcode, { data: completeData }] =
    useCompleteInputBarcodeMutation();
  ...
  const sizeInfoColumns = [
    ...
    },
    {
      ...
      flex: 1,
      renderCell: (row) => {
        const { sizeID, inputBalQty } = row?.row;
        return (
          <input
            type="text"
            maxLength="10"
  ...
  const handleAdd = (e) => {
    ...
    saveInputBarcode(payload);
  };

  // handleDelete function
  const handleDelete = (row) => {
    deleteInputBarcode(row?.btBundleNo);
  };
  ...
  useEffect(() => {
    ...
    if (isDeleteSuccess || isSaveSuccess) {
      refetchAddData();
      if (isDeleteSuccess) successToast("Deleted Successfully");
    }
  }, [isDeleteSuccess, isSaveSuccess, completeData]);
  ...
  return (
    <>
      <form onSubmit={handleAdd}>
        <CustomAppBar title={"Line Wise Barcode Generate"} />
      ...
      <Box sx={{ p: 1, border: "1px dashed grey" }}>
        ...
        <CustomTable
          ...
          hideFooter={true}
          pagePerSize={99}
          height={addView?.length > 0 && style ? "auto" : "180px"}
...
export default GenerateBarcodeInput;
