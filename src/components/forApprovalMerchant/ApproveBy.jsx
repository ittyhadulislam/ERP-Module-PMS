...
import { useLazyGetMerchantApproveByQuery } from "../../redux/features/gatePass/merchantForApproval/merchantForApprovalQuery";
...
const ApproveBy = ({ buttonData, company }) => {
  ...
  // get table data
  const [getApproveData, { data, isLoading }] =
    useLazyGetMerchantApproveByQuery();
  useEffect(() => {
    company && getApproveData(company?.nCompanyID);
  }, [company]);
  // approve mutation
  const [
    approve,
  ...
  useEffect(() => {
    ...
    approveSuccess && getApproveData(company?.nCompanyID);
  }, [approveData]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    cancelSuccess && getApproveData(company?.nCompanyID);
  }, [cancelData]);
  useEffect(() => {
