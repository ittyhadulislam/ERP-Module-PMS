...
import { useLazyGetMerchantConfirmByQuery } from "../../redux/features/gatePass/merchantForApproval/merchantForApprovalQuery";
...
const ConfirmBy = ({ buttonData, company }) => {
  ...
  // get table data
  const [getConfirmData, { data, isLoading }] =
    useLazyGetMerchantConfirmByQuery();
  useEffect(() => {
    company && getConfirmData(company?.nCompanyID);
  }, [company]);
  // confirm mutation
  const [
    confirm,
  ...
  useEffect(() => {
    ...
    confirmSuccess && getConfirmData(company?.nCompanyID);
  }, [confirmData]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    cancelSuccess && getConfirmData(company?.nCompanyID);
  }, [cancelData]);
  useEffect(() => {
