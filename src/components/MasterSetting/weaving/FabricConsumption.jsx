...
import { useGetWeavingMasterCompositionQuery } from "../../../redux/features/weaving/masterSetup/queryMasterSetup";
import { useSaveCompositionMutation } from "../../../redux/features/weaving/masterSetup/mutationMasterSetup";
...
const FabricConsumption = () => {
  ...
  //   get table data
  const { data, isLoading, refetch } = useGetWeavingMasterCompositionQuery();
  ...
  const handleSubmit = (e) => {
    ...
    const payload = {
      fCompositionID: 0,
      fabricCompositionName: name,
      createdBy: userName,
    };

    const updatePayload = {
      fCompositionID: selectedRow?.fcom_id,
      fabricCompositionName: name,
      createdBy: userName,
    };
    saveComposition(selectedRow ? updatePayload : payload);
  };
  ...
  useEffect(() => {
    if (selectedRow) {
      setName(selectedRow?.fcom_composition);
    }
  }, [selectedRow]);
  //   table columns
  const columns = [
