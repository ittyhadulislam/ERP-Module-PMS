...
import { useGetWeavingMasterYarnCountQuery } from "../../../redux/features/weaving/masterSetup/queryMasterSetup";
import { useSaveYarnCountMutation } from "../../../redux/features/weaving/masterSetup/mutationMasterSetup";
...
const YarnCount = () => {
  ...
  //   get table data
  const { data, isLoading, refetch } = useGetWeavingMasterYarnCountQuery();
  ...
  const handleSubmit = (e) => {
    ...
    const updatePayload = {
      yarnCountID: selectedRow?.yc_id,
      yarCounts: name,
      createdBy: userName,
    };
    saveYarnCount(selectedRow ? updatePayload : payload);
  };

  useEffect(() => {
    if (selectedRow) {
      setName(selectedRow?.yc_description);
    }
  }, [selectedRow]);
  //   table columns
  const columns = [
