...
import { useGetWeavingMasterGsmQuery } from "../../../redux/features/weaving/masterSetup/queryMasterSetup";
import { useSaveGsmMutation } from "../../../redux/features/weaving/masterSetup/mutationMasterSetup";
...
const GSM = () => {
  ...
  //   get table data
  const { data, isLoading, refetch } = useGetWeavingMasterGsmQuery();
  ...
  const handleSubmit = (e) => {
    ...
    const updatePayload = {
      gsmID: selectedRow?.gsm_id,
      gsm: name,
      createdBy: userName,
    };
    saveGsm(selectedRow ? updatePayload : payload);
  };

  useEffect(() => {
    if (selectedRow) {
      setName(selectedRow?.gsm_description);
    }
  }, [selectedRow]);
  //   table columns
  const columns = [
    ...
    {
      field: "gsm_description",
      headerName: "GSM",
      minWidth: 120,
      flex: 1,
    },
    {
      field: "gsm_created_by",
